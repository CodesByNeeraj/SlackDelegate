...
def get_tasks_created_by(workspace_id: str, creator_slack_id: str) -> list[dict]:
    """Returns all tasks created by this user in a workspace, sorted by created_at desc."""
    from boto3.dynamodb.conditions import Key, Attr
    table = get_table(TABLE_NAME)
    response = table.query(
        KeyConditionExpression=Key("workspace_id").eq(workspace_id),
        FilterExpression=Attr("created_by").eq(creator_slack_id),
    )
...
def get_task_by_dm_ts(workspace_id: str, dm_message_ts: str) -> dict | None:
    """
    Scans for the task whose DM thread matches this ts, scoped to workspace.
    """
    from boto3.dynamodb.conditions import Key, Attr
    table = get_table(TABLE_NAME)
    response = table.query(
        KeyConditionExpression=Key("workspace_id").eq(workspace_id),
        FilterExpression=Attr("dm_message_ts").eq(dm_message_ts),
    )
    items = response.get("Items", [])
